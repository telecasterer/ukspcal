...
export function getNavigatorStandalone(navigatorLike: unknown): boolean {
    return Boolean(
        (navigatorLike as { standalone?: boolean } | null | undefined)
            ?.standalone
    );
}
 ...
 */
export function getDisplayModeStandalone(
    matchMediaResult: MediaQueryList | null | undefined
): boolean {
...
export function computeIsStandalone(options: {
    ...
    const iosStandalone =
        isIosUserAgent(options.userAgent) && options.navigatorStandalone;
