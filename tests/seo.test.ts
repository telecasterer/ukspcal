...
describe('SEO - Meta Tags and Schema', () => {
	...
	it('should have page-specific title on help page', () => {
		expect(helpPageContent).toContain('<title>Help - UK State Pension Calculator');
	});

	it('should have Open Graph title on main page', () => {
		expect(mainPageContent).toContain('property="og:title"');
		expect(mainPageContent).toContain('UK State Pension Calculator - Payment Dates & Schedule');
	});
	...
	it('should have proper H1 on main page', () => {
		expect(mainPageContent).toContain('<h1 class="text-4xl font-bold');
