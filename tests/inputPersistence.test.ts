...
describe("inputPersistence", () => {
	...
	it("returns empty object for invalid JSON", () => {
		expect(parsePersistedInputsJson("{not json}", { allowedCycleDays, allowedDateFormats })).toEqual({});
	});
	...
	it("sanitizes fields and applies allow-lists", () => {
		...
		const parsed = parsePersistedInputsJson(raw, { allowedCycleDays, allowedDateFormats });
	...
	it("loadPersistedInputs is resilient to storage errors", () => {
		...
		expect(loadPersistedInputs(storage, "k", { allowedCycleDays, allowedDateFormats })).toEqual({});
	});
